...
default_opts = "https://torrentio.strem.fun/sort=qualitysize/manifest.json"
...
def get(url):
    ...
    try:
        ...
        ui_print(f"[torrentio] error: {e}", debug=ui_settings.debug)
        return None
...
def scrape(query, altquery):
    ...
    if type == "movie":
        ...
        ui_print("[torrentio debug] scraping url: " + url)
        response = get(url)
        ui_print(f"[torrentio debug] Response: {str(response)[:100]}")
    ...
    for result in response.streams:
        try:
            ...
            ui_print("[torrentio error] failed to parse release: " + str(e))
            continue
    ui_print(f"[torrentio debug] Found {len(scraped_releases)} releases")
    return scraped_releases
