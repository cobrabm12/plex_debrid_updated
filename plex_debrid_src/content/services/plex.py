...
def oauth(code=""):
    ...
    if code == "":
        ...
        data = {
            ...
            'X-Plex-Model': 'Plex Debrid',
            'X-Plex-Client-Identifier': client_identifier,
        }
        try:
            response = requests.post('https://plex.tv/api/v2/pins?strong=false', headers=headers, data=data)
            response = json.loads(response.content)
            return response['id'], response['code']
        except Exception as e:
            print("Plex OAuth Error: " + str(e))
            return None, None
    else:
        headers = {
        ...
        url = 'https://plex.tv/api/v2/pins/' + str(code)
        ui_print("Waiting for you to link the account on the web page...", debug=ui_settings.debug)
        while True:
            try:
                response = requests.get(url, headers=headers)
                data = json.loads(response.content)
                if data.get('authToken'):
                    ui_print("Token received!")
                    return data['authToken']
                time.sleep(2)
            except Exception as e:
                ui_print("Polling error: " + str(e), debug=ui_settings.debug)
                time.sleep(2)

def logerror(response):
    if not response.status_code == 200:
        if response.status_code == 404 and "watchlist" in response.url:
            if "metadata.provider.plex.tv" in response.url:
                # Suppress error for standard API as we have fallback
                return
            ui_print("Plex Watchlist not found (404). This is expected for Managed Users. Please use Trakt for your watchlist.", debug=ui_settings.debug)
        else:
...
def get_user_id(token):
    try:
        response = requests.get('https://plex.tv/api/v2/user', headers={'X-Plex-Token': token, 'Accept': 'application/json'})
        data = json.loads(response.content)
        return data['id']
    except:
        return None
...
class watchlist(classes.watchlist):
                ...
                while added < total:
                    ...
                    if hasattr(response, 'Error'):
                        ...
                        if hasattr(response, 'Error'):
                            ...
                            user_id = get_user_id(user[1])
                            if user_id:
                                rss_url = 'https://plex.tv/users/' + str(user_id) + '/watchlist.rss?X-Plex-Token=' + user[1]
                                try:
                                    ...
                                    if rss_resp.status_code == 200:
                                        ...
                                        for item in root.findall('./channel/item'):
                                            ...
                                            type_ = "movie" if "movie" in guid else "show"
                                            dummy = SimpleNamespace()
                                            dummy.ratingKey = guid
                                            dummy.user = [user]
                                            dummy.type = type_
                                            if type_ == 'show':
                                                self.data += [show(dummy)]
                                            elif type_ == 'movie':
                                                self.data += [movie(dummy)]
                                        ui_print("[plex] RSS fallback successful.", debug=ui_settings.debug)
                                        # Skip the rest of the loop for this user as we got data from RSS
                                        added = total = 1 
                                        continue
                                except Exception as e:
                                    ui_print("[plex] RSS fallback failed: " + str(e), debug=ui_settings.debug)
                            else:
                                 ui_print("[plex] Could not get user ID for RSS fallback.", debug=ui_settings.debug)
                            pass
    ...
    def update(self):
        ...
        try:
            for user in users:
                # Directly use Discover API as requested
                            ...
                            try:
                                ...
                                if rss_resp.status_code == 200:
                                    ...
                                    for item in root.findall('./channel/item'):
                                        ...
                                        dummy.user = [user]
                                        dummy.type = type_
                                        dummy.title = item.find('title').text
                                        
                                        # Check if new
                                        if not dummy in self.data:
                                            ui_print('[plex] item: "' + dummy.title + '" found in ' + user[0] + '`s watchlist (RSS)')
                                            update = True
                                            if dummy.type == 'show':
                                                self.data += [show(dummy)]
                                            elif dummy.type == 'movie':
                                                self.data += [movie(dummy)]
                                        
                                        new_watchlist += [dummy]
                            except Exception as e:
                                ui_print("[plex] RSS fallback failed in update: " + str(e), debug=ui_settings.debug)
                        pass
...
class library(classes.library):
    ...
    def __new__(self,silent=False):
        ...
        if len(list_) == 0:
            ui_print("[plex warning]: Your library seems empty. Proceeding anyway as requested.", debug=ui_settings.debug)
